...
#'
#' This function lists all the conflicts between packages in the microbiomedataset
...
#' @docType methods
#' @export
#' @return microbiomedataset conflicts
#' @examples
#' microbiomedataset_conflicts()
microbiomedataset_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", microbiomedataset_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "microbiomedataset_conflicts")
}

microbiomedataset_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "microbiomedataset_conflicts()"
  )
...
#' @export
print.microbiomedataset_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(microbiomedataset_conflict_message(x))
}
